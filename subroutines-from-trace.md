...
| --- | --- | --- |
| `10161..10203` | `IDENTPROPS` | Pop via `03277`, validation at `06424`, result tail jump to `03275`; syntax, macro and identifier-class selection retain BESM tag arithmetic. |
| `10564..10566` | `UPDATER` getter | `10663` validation, `10704` missing-function path, `03275` result. |
| `13451..13453` | `STACKLENGTH` | `03305` stack-depth arithmetic and `03275` result. |
| `15314..15321` | `LISTREAD` | `06343` reader, `15322` list traversal and `15344` non-list restoration. |
| `15354..15375` | `NUMBERREAD` | `06343` reader, `06740` negation, `03275` result and `03014` diagnostic `05010`. |

Original return/branch entries remain semantic dispatch entries. The expanded
successful fixture matches historical and instruction-only output, with zero
raw steps in these regions. Invalid-token and EOF probes are separate and do
not establish zero fallback in the diagnostic machinery. No historical quine
call counts were changed by this expansion.
...
| `01006` | — | Shared initialization continuation. | Clears the four `r7`-relative state words and calls `05405`; `03521` returns here through its saved `r14` link. The profiled `zone1224` session exercised 56 semantic entries. |
| `01107` | — | Native initial-dictionary recognition and dynamic collision-chain lookup entry. | Builds the original hash and frame state, recognizes all 184 initial packed names through the C++ descriptor snapshot for `01400..02737`, and returns the corresponding BESM record. Properties, values, and links remain memory-resident. An array miss follows the live word-two links for dynamic identifiers and reaches `05430` only for a genuine miss, preserving continuations `01140` and `01151`. |
...
| `03325` | `ISLIST` | POP value classifier wrapper. | Pops one value, preserves the existing `03330` classifier boundary, and selects the two established result continuations through `r13`; the expanded standard-function fixture exercised all three entries. |
...
| `03521` | — | Two-operand linked replacement entry. | Derives `r11` from the shifted input, preserves the caller in `r14`, calls `03506` for both selected operands, then enters `03516` before returning through `01006`. The profiled `zone1224` session exercised all four translated entries 56 times. |
...
| `04624` | — | Generated three-boundary update path. | Passes the `r7+0717` word through `17013` and `17021`, stores the returned value, and selects either the `r2+066` repeat or the `r2+053` continuation. The profiled `zone1224` session exercised all three entries 57 times. |
...
| `06611` | `ISWORD` | Masked POP value-tag predicate. | Pops one value, applies the original `06662` mask and `06666` XOR, and selects the existing `06614` or `06616` result path without absorbing either stack/environment return. |
...
| `07367` | `SAMEDATA` | Evaluated-word identity predicate. | Evaluates both operands independently through `02767`, saves the first through `03303`, pops the second through `03277`, then XOR-compares the complete words and selects `01637` or `02207` before the existing `03301` stack/environment path. |
| `07433` | `BOUNDSLIST` | Array-bounds list constructor. | Preserves the two POP argument transfers and evaluator boundary, validates the returned descriptor with the resident mask, then follows the bounds-record chain and allocates its two list cells per record through separate `05215` calls. Continuations `07453`, `07455`, and `07460` retain the construction loop and restore `r1`, `r2`, and the hardware stack; invalid objects retain diagnostic `12100` at `07463`. |
...
| `07652`, `07667` | — | Callback sequence and alternate value entry. | `07652` saves `r1`, `r5`, and the caller, repeatedly dispatches the descriptor at `01567` through `02750`, and restores the frame. `07667` obtains one POP value, passes it through `07673`, and enters `03235`. The profiled `zone1224` session made 88 semantic dispatches in the combined region. |
...
| `10350` | `ISLINK` | POP value link predicate wrapper. | Pops one value, preserves the existing `10353` classifier boundary and its nested `17045` path, then returns the result through the ordinary `03275`/`03235` bracket. |
| `10402` | `COREUSED` | Free-list accounting routine. | Follows the allocator chain rooted through `05502`, sums each word's upper-half extent with BESM address arithmetic, and forms the free and used tagged results with the original `NTR 3` reverse subtraction before two separate POP-stack returns. Adjacent entries `10416` and `10417` preserve the two evaluator wrappers newly reached by the appendix fixture. |
| `10421` | `FNCOMP` | Function-composition descriptor constructor. | Saves `r1` and `r2`, validates both function tags, allocates the five-word descriptor through `05430`, publishes the tagged function through `03275`, allocates its helper through `11464`, installs both source functions, and restores the complete hardware-stack frame. Invalid operands retain diagnostic continuation `10443`. |
| `10604..10606` | `FNPART` | Function-part getter. | Calls the existing function-object validator at `10672`, loads field `+3` from the validated object, and returns it through the independent `03275` POP-stack boundary. Entries `10605` and `10606` retain the validator continuation and final tail transfer explicitly. |
...
| `11647` | — | Generated arithmetic update entry. | Saves a three-word frame, pops the operand through `03277`, preserves the original BESM tag tests and arithmetic path, calls `11464`, and completes through the generated continuation at `11514`. The profiled `zone1224` session exercised the entry and both call continuations six times. |
...
| `14651..14721` | — | Library-loader setup, name-copy loop, and restoration path. | Saves the evaluator frame, validates and copies the two catalog names through `21431`, `21275`, and `21443`, enters native directory handling at `14662`, and restores `r1..r3` at `14700`. The native-library CTest profile formerly executed 139 raw steps here and now executes none. |
| `15712..15750` | `CODIPS`, `CODPIS` | Packed-string character-code conversion. | Entries `15712` and `15713` select `21275` or `21274`, validate both `162`-tagged strings, form the original descriptors at `15752..15753`, and convert the shorter string length through the independent `21431`, selected character-converter, and `21443` boundaries. Empty strings, diagnostic `11700`, and complete `r1`, `r4`, `r15`, and hardware-stack restoration remain distinct continuations. |
| `15754..15762` | `CODIPC`, `CODPIC` | Single-character code conversion. | Selects `21275` or `21274`, passes the character through that independent converter boundary, retags the result with the original constant at `01637`, replaces the POP argument in place, and restores the saved caller from `r17`. |
...
| `16610..16640` | — | Record comparison callbacks and bounded shifted-word loop. | Preserves calls to `16421`, `02750`, `03277`, `16505`, and `16605`, computed returns through `r7`, and relocated exits through `r1`. The language-coverage test formerly executed 108 raw steps in this block and now dispatches every observed entry semantically. |
| `16643..16664` | — | Generated record paths, decimal setup, and real-result exit. | Preserves the `03275`, `16742`, and `16605` boundaries, the `r17-7` cyclic update, the original `r5=014` counter, the diagnostic exit, and the relocated continuations. At `16645`, the real scanner's retained external-code spelling at `r1+74276` is parsed natively; decimal formation still uses the image's BESM `0.1` and exponent constants so the historical truncated 48-bit results are retained. The profiled `zone1224` session made 284 semantic dispatches in this region and no raw instruction steps there. |
| `16672..16735` | — | Fractional and `$`-exponent real-literal scanner. | Accumulates fractional digits with the image's `0.1`, recognizes optional exponent signs, accumulates the exponent with the original cyclic arithmetic, and applies the `16736`/`16737` factors through BESM multiplication. Calls through `16605`, `16505`, and `16742` remain separate semantic boundaries; successful conversion rejoins the native result exit at `16645`. |
...
| `17077..17112` | — | Output descriptor comparison and restoration loop. | Saves `r1` and the caller, compares the words selected through `03637` and `03641`, repeats through `17070`, and restores both stack words at `17111`. The call-separated continuation at `17101` remains an independent semantic entry. |
| `17150` | — | Generated descriptor-construction entry. | Saves `r1` and the caller, preserves calls through `25556`, `17571`, two `04447` boundaries, and `17070`, then restores the frame and enters `25532`. The profiled `zone1224` session exercised six translated boundaries five times each. |
| `17175` | — | Generated table-initialization entry. | Saves `r1..r3`, calls `05007`, updates the table selected through `17562` and `17341`, initializes the record areas through `03531`, and restores the saved compiler frame. The profiled `zone1224` session exercised five complete traversals. |
...
| `25532` | — | Generated descriptor-frame builder. | Stacks the words at `04463`, `03645`, `17560`, and `17577`, invokes `17614` for both descriptor areas, rewrites the shared scratch words, and transfers to generated code at `32535`. The profiled `zone1224` session exercised all three static entries five times. |
...
Historical dispatch counts below remain measurements of the revisions that
produced them and have not been rewritten.
...
boundaries.
The following recommended sequence converts `03521..03530` with shared return
`01006..01010`, `11647..11672`, `17175..17231`, `17150..17165` with
`25532..25553`, `04624..04630`, and `07652..07672`. The current `zone1224`
profile executes zero raw instructions in all eight ranges and totals 229,570
semantic dispatches plus 106,962 raw instructions. The quine profile falls to
11,341 semantic dispatches plus 285 raw instructions. Generated transfers such
as `25553 -> 32535` remain instruction-interpreted boundaries.
