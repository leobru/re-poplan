...
|---:|---|---|
| `01107` | `p01107` | Build the observed hash value, recognize initial dictionary names from the native descriptor snapshot, search live dynamic collision-chain records, and allocate a record when absent |
...
`src/poplan.cpp` mirrors all 184 initial four-word records at
`01400..02737` in a native descriptor array. `01107` retains the original hash
and frame setup, then recognizes an initial packed name from this snapshot and
returns its original BESM record address. Only the name is authoritative in
the snapshot: properties, values, and links remain live in BESM memory.

Names absent from the initial array still follow the selected live word-two
collision-chain links, so dynamically interned identifiers remain visible. A
genuine miss still returns through `05430` and the original `01140` or `01151`
